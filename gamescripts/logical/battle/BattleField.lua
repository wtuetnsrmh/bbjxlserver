...
function BattleField:ctor(params)
	...
	self.gridWidth = params.gridWidth or 140
...
function BattleField:getXposTeamer(soldier, xNo)
	...
	if xPosOne == soldier then return nil end
...
function BattleField:getSideObjects(centerSoldier)
	...
	for anchKey, soldier in pairs(self[centerSoldier.camp .. "SoldierMap"]) do
		if centerSoldier:getAnchKey() ~= soldier:getAnchKey() then
			...
			if math.abs(xPos - centerSoldier.position.x) <= self.gridWidth / 2 then
...
function BattleField:getCampObjects(camp)
	return table.values(self[camp .. "SoldierMap"])
end
...
function BattleField:calculateGameResult()
	...
	-- 战斗胜利
	local starNum = battleResultCsv.winBattle
	
	-- 阵亡武将数目
	if table.nums(self.leftSoldierMap) * 100 / self.leftSoldierNum >= battleResultCsv.deadSoldierRatio then
		starNum = starNum + 1
	end

	if table.nums(self.leftSoldierMap) == self.leftSoldierNum then
		starNum = starNum + 1
	end
