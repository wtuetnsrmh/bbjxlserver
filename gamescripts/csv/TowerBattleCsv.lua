...
function TowerBattleCsvData:getCarbonAwardData(carbonId)
	local function getDropItem(itemMap)
		...
		for _, itemData in pairs(itemMap) do
			table.insert(array, { itemId = itemData[1],itemNum = itemData[2], weight=itemData[3] })
		end
	...
	local awardData, awardArr = {}, {}
	while #awardData == 0 do
		local item, itemNum = getDropItem(self.m_data[carbonId].mustItems)
		if item then
			table.insert(awardData,{ itemId = tonum(item), num = tonum(itemNum) })
			awardArr[tostring(item)] = tonum(itemNum)
		end
	end
	
	while #awardData < 4 do
		local item, itemNum = getDropItem(self.m_data[carbonId].otherItems)
		if item and not awardArr[tostring(item)] then			
			table.insert(awardData,{ itemId = tonum(item), num = tonum(itemNum) })
			awardArr[tostring(item)] = tonum(itemNum)
		end
	end

	return awardData, awardArr
end
