...
function LegendShopCsv:load(fileName)
	self.m_data = {}
	...
	for line = 1, #csvData do
		...
		if id > 0 then
			self.m_data[id] = {
				...
				soulPrice = tonum(csvData[line]["将魂单价"]),
...
function LegendShopCsv:getShopData(id)
	return self.m_data[id]
end

function LegendShopCsv:randomShopIds()
	local selectedIds = {}
